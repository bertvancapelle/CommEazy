/**
 * ContactReachabilityIcons — Three fixed-position status icons under a contact name
 *
 * Shows communication channel availability for a contact:
 * - Position 1 (left): CommEazy app (chatbubble icon)
 * - Position 2 (middle): Email (mail icon)
 * - Position 3 (right): Phone (phone icon)
 *
  ...
  return (
    <View
      ...
      accessibilityLabel={t('contacts.reachability.a11ySummary', {
        ...
        phone: hasPhone ? t('contacts.reachability.available') : t('contacts.reachability.unavailable'),
      })}
    >
      ...
      {/* Position 3: Phone */}
      <Icon
        name={hasPhone ? 'phone' : 'x'}
        size={size}
        color={hasPhone ? themeColors.success : themeColors.error}
      />
    </View>
  );
}
